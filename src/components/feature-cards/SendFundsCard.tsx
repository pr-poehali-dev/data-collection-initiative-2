import { Zap, ChevronDown, ArrowUpRight } from "lucide-react"
import { Switch } from "@/components/ui/switch"
import Icon from "@/components/ui/icon"

export function SendFundsCard() {
  return (
    <div className="rounded-2xl bg-[#141414] border border-[#262626] p-6 flex flex-col">
      <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl bg-[#1f1f1f] border border-[#2a2a2a]">
        <Zap className="h-5 w-5 text-orange-400" />
      </div>

      <h3 className="mb-2 text-lg font-semibold text-white">Мгновенная установка</h3>
      <p className="mb-4 text-sm text-gray-400">Один клик — и плагин уже работает на вашем сайте без ручной настройки</p>

      <a href="#" className="mb-6 inline-flex items-center text-sm text-orange-400 hover:text-orange-300 transition-colors">
        Подробнее <ArrowUpRight className="ml-1 h-4 w-4" />
      </a>

      <div className="mt-auto space-y-4 rounded-xl bg-[#1a1a1a] border border-[#262626] p-4">
        <div className="flex items-center justify-between rounded-lg bg-[#0f0f0f] border border-[#262626] px-3 py-2.5">
          <div className="flex items-center gap-3">
            <Icon name="Globe" fallback="Box" className="h-5 w-5 text-gray-500" />
            <div>
              <p className="text-sm font-medium text-white">Мой сайт</p>
              <p className="text-xs text-gray-500">mysite.ru · WordPress 6.4</p>
            </div>
          </div>
          <ChevronDown className="h-4 w-4 text-gray-500" />
        </div>

        <div>
          <label className="mb-2 flex items-center gap-1 text-xs text-gray-400">
            Поиск плагина
          </label>
          <div className="flex items-center rounded-lg bg-[#0f0f0f] border border-[#262626] px-3 py-2.5">
            <Icon name="Search" fallback="Box" className="h-4 w-4 text-gray-500 mr-2" />
            <input
              type="text"
              placeholder="SEO, формы, безопасность..."
              className="flex-1 bg-transparent text-white placeholder-gray-600 outline-none text-sm"
            />
          </div>
        </div>

        <div>
          <label className="mb-2 flex items-center gap-1 text-xs text-gray-400">
            Примечание к установке <span className="text-orange-400">*</span> (Необязательно)
          </label>
          <div className="relative">
            <textarea
              placeholder="Дополнительные настройки или пожелания..."
              className="w-full rounded-lg bg-[#0f0f0f] border border-[#262626] px-3 py-2.5 text-sm text-white placeholder-gray-600 outline-none resize-none h-16"
            />
            <span className="absolute bottom-2 right-2 text-xs text-gray-600">0/200</span>
          </div>
        </div>

        <div className="flex items-center justify-between pt-2">
          <Switch className="data-[state=checked]:bg-orange-500" />
          <span className="text-sm text-gray-400">Автообновление плагина</span>
        </div>
      </div>
    </div>
  )
}
